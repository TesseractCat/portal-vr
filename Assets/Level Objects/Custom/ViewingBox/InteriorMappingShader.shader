...
Shader "Volume/InteriorMapping"
{
	Properties
	{
		...
		_FloorDist("Distance Between Floors", Float) = 1.3
		_WallDist("Distance Between Walls", Float) = 1
		_BackWallDist("Distance To Back Wall", Float) = 1
		_BackWallTexture("Back Wall Texture", 2D) = "defaulttexture" {}
	}
...
#pragma target 3.0
	...
	//Textures
	sampler2D _BackWallTexture;
	float4 _BackWallTexture_ST;
	...
	float4 checkIfCloser(float3 rayDir, float3 rayStartPos, float3 planePos, float3 planeNormal, float4 color, float4 colorAndDist)
	{
		...
		if (t < colorAndDist.w)
		{
			...
			colorAndDist.rgb = tex2D(_BackWallTexture, intersectPos.xy * _BackWallTexture_ST.xy + _BackWallTexture_ST.zw) * color;
		}
	...
	void surf(Input IN, inout SurfaceOutputStandard o)
	{
		...
		if (dot(upVec, rayDir) > 0)
		{
			...
			float3 wallPos = ((ceil(rayStartPos.y / _FloorDist) - 0.0) * _FloorDist) * upVec;
		...
		{
			float3 wallPos = ((ceil(rayStartPos.y / _FloorDist) - 1.0) * _FloorDist) * upVec;
		...
		{
			float3 wallPos = (ceil(rayStartPos.x / _WallDist) * _WallDist) * rightVec;

			colorAndDist = checkIfCloser(rayDir, rayStartPos, wallPos, rightVec, _ColorWall, colorAndDist);
		}
		else
		{
			float3 wallPos = ((ceil(rayStartPos.x / _WallDist) - 1.0) * _WallDist) * rightVec;
		...
		{
			float3 wallPos = ((ceil(rayStartPos.z / _WallDist) + 1.0) * _BackWallDist) * forwardVec;
